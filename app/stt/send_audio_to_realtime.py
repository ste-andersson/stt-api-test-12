...
def init_audio_sender_state(*, idle_timeout_s: float = 2.0) -> Dict[str, Any]:
    """
    Minimal statecontainer som motsvarar has_audio/last_audio_time/commit_task
    som tidigare låg i main.py.
    """
    return {
        "has_audio": False,
        "last_audio_time": 0.0,
...
async def start_commit_loop(*, state: Dict[str, Any], rt, commit_interval_ms: int, logger) -> asyncio.Task:
    """
    Oförändrad commit-policy extraherad från main.py:
      - sov commit_interval_ms
      - committa bara om vi har skickat ljud
      - om idle längre än idle_timeout_s: återställ flagga
      - hantera 'buffer too small' / 'input_audio_buffer_commit_empty' mjukt
    """
...
async def handle_audio_chunk(*, state: Dict[str, Any], rt, buffers, chunk: bytes, logger) -> None:
    """
    Oförändrad 'append'-del extraherad från main.py:
      - skicka chunk till Realtime
      - logga längd till buffers.openai_chunks
      - uppdatera has_audio/last_audio_time
    """
